...
export default class Logger {
  ...
  public static init() {
    const folder = process.env.REPORTING_FOLDER ?? "data/";
    if (!fs.existsSync(folder))                    fs.mkdirSync(folder, { recursive: true });
    if (!fs.existsSync(`${folder}logs`))           fs.mkdirSync(`${folder}logs`);
    if (!fs.existsSync(`${folder}errors`))         fs.mkdirSync(`${folder}errors`);

    fs.appendFileSync(Logger.logFileName, `timestamp|scope|type|content\n`);
  }
